...
const Course = ({course}) => {
     const {name , img , rating , description , id}= course
    return (
        <div className='p-2 rounded backdrop-blur-[2px] backdrop-contrast-125  backdrop-opacity-10 bg-white/20 relative hover:drop-shadow-2xl'>
            ...
            <div className='mt-2'>
                ...
                <p className='text-lg text-white'>{description}</p>
                 ...
                 <Link  to='/course' className='absolute bottom-2 right-2.5 text-white px-6 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full drop-shadow-lg'>
                    Enroll Now
                </Link >
            </div>
        </div>
